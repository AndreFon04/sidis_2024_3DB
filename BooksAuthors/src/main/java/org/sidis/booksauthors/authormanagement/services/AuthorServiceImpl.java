...
public class AuthorServiceImpl implements AuthorService {
    ...
    @Override
    public Map<String, Long> getTop5AuthorsLendings() {
        List<Object[]> results = authorRepository.getTop5AuthorsLending(PageRequest.of(0, 5));
        Map<String, Long> top5AuthorsLending = new HashMap<>();
        for (Object[] result : results) {
            String author = (String) result[0];
            Long count = (Long) result[1];
            top5AuthorsLending.put(author, count);
        }

        return top5AuthorsLending.entrySet().stream()
