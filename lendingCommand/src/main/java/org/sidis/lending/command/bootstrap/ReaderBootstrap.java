...
public class ReaderBootstrap implements CommandLineRunner {
    ...
    public void run(final String... args) throws Exception {
        ...
        final Optional<ReaderL> r = readerRepo.findTopByOrderByReaderIDDesc();
        if (r.isPresent()) {
        ...
        if (readerRepo.findByEmail("josefinoDasCOUBES@email.com").isEmpty()) {
            ...
            final ReaderL r1 = new ReaderL("Josefino Amigalhaço das Coubes", encoder.encode("Password1"), "josefinoDasCOUBES@email.com", "1991-1-1", "946325614", true);
            readerRepo.save(r1);
    ...
    private void addReader(final String name, final String password, final String email, final String birthdate, final String phoneNumber, final boolean GDPR) {
        if (readerRepo.findByEmail(email).isEmpty()) {
            ReaderL reader = new ReaderL(name, password, email, birthdate, phoneNumber, GDPR);
